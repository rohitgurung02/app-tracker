...
const Map = ({ locations = [] }) => {
  ...
  // const API_KEY = "2150ddca088043798030e43de2798f3c"; // OpenCage API Key
  ...
  const fetchAddress = async (latitude, longitude) => {
    try {
      ...
      if (data.results && data.results.length > 0) {
        setUserAddress(data.results[0].formatted); // Set the formatted address
      } else {
  ...
  return (
    <div>
      ...
      <h2>User Address</h2>
      {userAddress ? <p>{userAddress}</p> : <p>Fetching address...</p>}
